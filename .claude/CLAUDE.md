...
**WIP / stubbed / broken:**
- The **AI assistant workload itself is absent** (see §2.1). **Kyverno, and the kagent trio (Kagent + kmcp + agentgateway), are decided-but-unimplemented** — the trio is the chosen agentic stack (§2.2) but no Helm apps / manifests exist yet. The **A2A client (in the `enterpriseclaw` CLI)** is likewise net-new.
