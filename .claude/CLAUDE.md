...
**Decided spine:**
> Slack → Istio gateway → **Argo Events Slack EventSource** → Sensor → **Argo Workflow** (one short-lived run *per inbound message*) → **Step 0 — resolve the human's identity: call the Session-Broker reader (`POST /identity/resolve`, header `X-Slack-User-Id`; mTLS-gated so only this workflow-step's SA may call it)** → step calls **Kagent** over **A2A** (HTTP `:8083`, via an A2A-client container in the `actions/` pattern), **carrying the user's Keycloak access token (user identity) alongside the step's SPIFFE/mTLS principal (workload identity)** → agent reasons on **Claude via AWS Bedrock** (auth: **EKS IRSA** on its ServiceAccount — Bedrock supplies the LLM brain + guardrails only) → agent **opens a PR to the private gitops repo via the GitHub MCP server** (mutating ops behind a human-approval prompt) → reply posted back to Slack → human merges → **Argo CD** syncs → **Crossplane** reconciles the Claim → app Deployment (instant) + managed dependency (pre-warmed).
...
- **Two-identity propagation (the demo's headline).** **User identity** = the Keycloak **JWT** (scopes/roles); **workload identity** = the ambient-mesh **SPIFFE/mTLS** principal. Enforced by **Istio** `RequestAuthentication` (JWKS from Keycloak) + `AuthorizationPolicy` in front of each agent/MCP, combining `from.source.principals` (workload) **and** `when.request.auth.claims` (user scopes) — *who **and** what*. The JWT scopes — not Bedrock — decide which agents/MCPs the human may invoke.
- **Per-message branch (Step 0).** **Token found** → carry user JWT + workload SPIFFE into the A2A call (spine above). **No token** → a **general (read-only-scope) agent triages intent**: answerable informationally → just answer, no login; privileged action implied (review an issue, open a PR, provision infra) → call the broker **writer** `POST /auth/login/start` → post the returned Keycloak `/authorize` URL to Slack → end the run. The next Slack reply re-fires a fresh workflow with the token now cached. The login wall is **conditional on intent**, not on mere auth state — the `AuthorizationPolicy` requiring a JWT on privileged agents/MCPs is what forces it.
- **Ownership boundary.** **Keycloak + Redis + Dapr and the OAuth write/read paths live in the Session-Broker repo, not here** (the Keycloak helm-app left this repo in `796d38e`). EnterpriseClaw owns Argo Events/Workflows, Istio, Kagent, Crossplane, and the enforcement `AuthorizationPolicy` objects. The agent calls the broker, **never Keycloak directly** — the broker is the only component that touches Keycloak on the write path.
- **Subtlety to resolve before building (flagged, not solved):** the user's bearer is propagated across hops (workflow step → Kagent → MCP), so each target needs its own `RequestAuthentication` and a decision on **`aud` handling** (one broad audience vs. per-agent token exchange).

**Build phases** (do P0 first — riskiest unknown): **P0** Kagent install + `ModelConfig` (Bedrock) + IRSA role + A2A reachability · **P1** wire the GitHub MCP server + test the PR-open path end-to-end · **P2** Crossplane + AWS provider + a minimal **XRD/Composition** Golden Path, landing where an ApplicationSet globs it · **P3** stitch Slack↔Workflow↔agent + **Session-Broker Step 0 (`/identity/resolve`) + the no-token login-wall (`/auth/login/start`) + Istio `RequestAuthentication`/`AuthorizationPolicy` enforcement** + archive/artifacts + pre-warm the slow managed dependency.
