...
**WIP / stubbed / broken:**
- The **AI assistant workload itself is absent** (see §2.1) — no `Agent`/`ModelConfig`/`MCPServer` CRs or agent logic yet. But the **kagent-trio install scaffolding now EXISTS** (added 2026-06-24): Helm apps under [gitops/helm/agents/](../gitops/helm/agents/) — `kagent`+`kagent-crds` (0.9.9), standalone `kmcp`+`kmcp-crds` (0.3.0), and `agentgateway`+`agentgateway-crds` (`cr.agentgateway.dev/charts` @ 1.3.1, ns `agentgateway-system`, `istio.autoEnabled: true`) — delivered via an ApplicationSet and **verified healthy on the dry-run VM**. Version/CRD/values specifics live in the [kagent-trio skill](skills/kagent-trio/). **Kyverno is still decided-but-unimplemented**, and the **A2A client (in the `enterpriseclaw` CLI)** is still net-new.
